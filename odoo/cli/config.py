...
class ConfigCommand(Command):
    ...
    def run(self, args):
        ...
        if params.database:
            ...
            raise NameError(
                "No database specified tue parameter or config file!")
    ...
    def run_test(self,
        ...
        for m in mods:
            ...
            if suite.countTestCases():
                ...
                if time.time() - t0 > 5:
                    _logger.log(25, "%s tested in %.2fs, %s queries",
                                m.__name__,
                                time.time() - t0,
                ...
                if not result.wasSuccessful():
                    ...
                    _logger.error("Module %s: %d failures, %d errors",
                                  module_name, len(result.failures),
                                  len(result.errors))
        ...
        if modules:
            ...
            for module_name in modules:
                ok = self.run_test(module_name, test_prefix, test_case,
                                   test_tags, test_position) and ok
            if ok:
...
class Install(Command):
    ...
    def run(self, args):
        ...
        def setupAddons(onlyLinks=False):
            ...
            linkedBaseEntries = linkDirectoryEntries(dir_odoo_addons,
                                                     lib_path_addons)
                ...
                for curAddonPackageDir in glob.glob(curPattern):
                    ...
                    if not curAddonPackageDir in dir_processed:
                        ...
                        if os.path.isdir(curAddonPackageDir):
                            # get include list
                            addonIncludeList = includeAddons.get(
                                packageName, None)
                            # process addons
                            for curAddon in listDir(curAddonPackageDir):
                                if not curAddon in ignoreAddons and (
                                        addonIncludeList is None
                                        or curAddon in addonIncludeList):
                                    curAddonPath = os.path.join(
                                        curAddonPackageDir, curAddon)
                                    for manifest_name in MANIFEST_NAMES:
                                        curAddonPathMeta = os.path.join(
                                            curAddonPath, manifest_name)
                                        if os.path.exists(curAddonPathMeta):
                                            addonMeta = None
                                            with open(curAddonPathMeta
                                                      ) as metaFp:
                                                addonMeta = eval(metaFp.read())

                                            # check api
                                            supported_api = addonMeta.get(
                                                "api")
                                            if not supported_api or ADDON_API in supported_api:
                                                dstPath = os.path.join(
                                                    dirEnabledAddons, curAddon)
                                                if not os.path.exists(
                                                        dstPath
                                                ) and not curAddonPath.endswith(
                                                        ".pyc"):
                                                    # log.info("Create addon link " + str(dstPath) + " from " + str(curAddonPath))
                                                    os.symlink(
                                                        curAddonPath, dstPath)
...
class Serve(Command):
    ...
    def run(self, cmdargs):
        ...
        parser.add_argument("--create",
                            action="store_true",
                            help="Create databse if it not exist")
        parser.add_argument(
            "--path",
            help=
        ...
        if "--addons-path" not in cmdargs:
            addon_pattern = [
                dir_server + "/addons", dir_workspace + "/addons*"
            ]
            package_paths = set()
            for cur_pattern in addon_pattern:
                for package_dir in glob.glob(cur_pattern):
        ...
        if args.db_name or args.create:
            ...
            if args.create:
                try:
                    ...
                    die("Could not create database `%s`. (%s)" %
                        (args.db_name, e))
